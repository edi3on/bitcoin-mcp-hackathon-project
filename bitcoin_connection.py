...
class BitcoinConnection:
    ...
    def __init__(self, cli_path: str = None, datadir: str = None, conf: str = None, network: str = None):
        ...
        # We'll keep the original datadir for reference in get_connection_info
        self.original_datadir = self.datadir
    ...
    def run_command(self, command: List[str]) -> Dict[str, Any]:
        ...
        if self.datadir:
            # For signet, we still use the main datadir but add the -signet flag
            # Bitcoin Core will automatically look in the signet subfolder
            full_command.extend(["-datadir=" + self.datadir])
            
        if self.conf:
            full_command.extend(["-conf=" + self.conf])
            
        # Add network flags if specified
        if self.network == "signet":
            full_command.extend(["-signet"])
